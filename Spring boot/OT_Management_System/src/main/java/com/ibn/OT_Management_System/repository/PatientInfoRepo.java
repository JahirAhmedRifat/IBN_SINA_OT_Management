...
public interface PatientInfoRepo extends JpaRepository<PatientInfo,Long> {
    ...
    @Query(value = "SELECT * FROM patient_info WHERE flag IN ('OI', 'OC') ORDER BY last_modified_date DESC", nativeQuery = true)
    List<PatientInfo> getAllOIAndOCPatientInfo();

    @Query(value = "SELECT * FROM patient_info WHERE flag IN ('OO') ORDER BY last_modified_date DESC", nativeQuery = true)
